...
def main():
    ...
    tmp =[]
    """f = False
    for i in x.get_distros_dict():
        for j in dict_light:
            if i["lighting"] not in j:
                f= True
                break
        if(f == False):
            tmp.append(i["lighting"])

    print(tmp)"""
    for i in x.get_by_type_neighborho("א","כל-המתקנים"):
        tmp.append(i)
        print(i)
    print()

    tmp = modular_filtering(tmp,"lighting","כן")
    for i in tmp:
        print(i)
...
'''
    has_n = False
    for x in tmp:
        for y in lst_neighborho.values():
            if(x  in y):
                has_n = True
                break
        if(has_n == False):
